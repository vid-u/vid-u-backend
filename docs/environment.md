...
- **Meta (Facebook Login):** `META_APP_ID`, `META_APP_SECRET`, `META_REDIRECT_URI` (must be `https://<tunnel>/oauth/facebook/callback` in local dev — **not** `localhost`; Meta rejects `localhost` in App Domains). Register the tunnel host in **App Domains** and **Valid OAuth Redirect URIs**. After callback, the API redirects to `{FRONTEND_URL}/account?oauth=…&platform=…` (first `FRONTEND_URL` origin; `http://localhost:5173` is fine). OAuth scopes requested by the API: `public_profile`, `user_videos`, `user_posts`, `pages_show_list`, `pages_read_engagement`, `read_insights` — add the same permissions under **Facebook Login → Permissions and features** in the Meta app (`user_videos` / `user_posts` may require Meta app review; creators must **reconnect** Facebook after scope changes). Reel/Watch stats: user token for personal Reels, then Page tokens from `/me/accounts` for Page Reels (`video_insights`). Instagram post URLs are not verified via this connect flow.
