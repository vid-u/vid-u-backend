...
- **TikTok:** `TIKTOK_CLIENT_KEY`, `TIKTOK_CLIENT_SECRET`, and `TIKTOK_REDIRECT_URI` (must match Login Kit redirect URI in the developer portal). TikTok does **not** allow `http://localhost` callbacks — use an **HTTPS tunnel** (e.g. `ngrok http 3001`) and set `TIKTOK_REDIRECT_URI` to `https://<tunnel>/oauth/tiktok/callback`. PKCE `code_verifier` is embedded in the signed `state` JWT (cookie is a fallback). Flow: `GET /oauth/tiktok/start` (Bearer) → TikTok → `GET /oauth/tiktok/callback` on the **tunnel host** → redirect to `{FRONTEND_URL}/account?oauth=…` (`http://localhost:5173` is fine for the SPA). Keep `VITE_API_URL` on `http://localhost:3001` if the tunnel forwards to the same API process. Login Kit scopes requested by the API: `user.info.basic`, `user.info.profile` (for `username`), and `video.list` (submission stats). Add the same scopes in the TikTok developer portal; `user.info.stats` is optional and not requested by VidU today.
- **Meta (Facebook, dual-app):** VidU uses two Meta apps when `META_PAGE_APP_ID` and `META_PAGE_APP_SECRET` are set:
  - **Login app** (`META_LOGIN_APP_ID` / `META_LOGIN_APP_SECRET`, or legacy `META_APP_ID` / `META_APP_SECRET`): use case **Facebook Login**; scopes `public_profile`, `user_videos`, `user_posts`; redirect `…/oauth/facebook/callback`.
  - **Page app** (`META_PAGE_APP_ID` / `META_PAGE_APP_SECRET`): use case **Manage everything on your Page**; enable `read_insights` + `pages_read_engagement`; scopes `public_profile`, `pages_show_list`, `pages_read_engagement`, `read_insights`, **`business_management`** (required for `/me/accounts` since Graph API v19 — request Advanced Access in App Review); redirect `…/oauth/facebook/page/callback`.
  - `META_REDIRECT_URI` base (e.g. `https://<tunnel>/oauth/facebook/callback`) — both callback paths must be registered on the matching Meta app.
  Connect flow: creator hits `GET /oauth/facebook/start` → Login app → (if dual) `GET /oauth/facebook/page/start` → Page app → SPA `{FRONTEND_URL}/account?oauth=success&platform=facebook`.
  Single-app mode works if only login credentials are set (no Page insights). Creators must **reconnect** after credential or scope changes. Instagram post URLs are not verified via this connect flow.
