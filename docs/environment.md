...
- **Meta (Facebook, dual-app):** VidU uses two Meta apps when `META_PAGE_APP_ID` and `META_PAGE_APP_SECRET` are set:
  ...
  - **Page app** (`META_PAGE_APP_ID` / `META_PAGE_APP_SECRET`): use case **Manage everything on your Page**; enable `read_insights` + `pages_read_engagement`; scopes `public_profile`, `pages_show_list`, `pages_read_engagement`, `read_insights`; redirect `…/oauth/facebook/page/callback`.
