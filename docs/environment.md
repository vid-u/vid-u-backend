...
- `XENDIT_WEBHOOK_TOKEN`: callback verification token (`x-callback-token` on `POST /webhooks/xendit`). See **`.env.example`** for the full dashboard checklist. Register **one URL** (e.g. `https://{API}/webhooks/xendit`) for **both** Xendit products: **INVOICES** (invoice paid / `PAID`) and **DISBURSEMENT** (Payouts v2 succeeded & failed). **Campaign funding** is applied when the invoice webhook succeeds or when the brand uses **Apply credit** (`POST /brands/campaigns/:id/checkout/:externalId/sync`) — the browser redirect alone does not credit the campaign.
