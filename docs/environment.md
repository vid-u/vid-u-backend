...
- **Meta (Facebook):** `META_APP_ID`, `META_APP_SECRET`, `META_REDIRECT_URI` (must be `https://<tunnel>/oauth/facebook/callback` in local dev — **not** `localhost`; Meta rejects `localhost` in App Domains). Register the tunnel host in **App Domains** and **Valid OAuth Redirect URIs**. After callback, the API redirects to `{FRONTEND_URL}/account?oauth=…&platform=…` (first `FRONTEND_URL` origin; `http://localhost:5173` is fine). **Scopes must match your Meta app use case** (set `META_OAUTH_SCOPES` if needed):
  - **Facebook Login** app (default VidU): enable `user_videos` and `user_posts` in the dashboard; API requests `public_profile,user_videos,user_posts`. Do **not** request `pages_show_list` / `read_insights` on this app type — Meta shows *Invalid Scopes*.
  - **Manage everything on your Page** app (separate Meta app only): enable `read_insights` and `pages_read_engagement`; set `META_OAUTH_SCOPES=public_profile,pages_show_list,pages_read_engagement,read_insights`. Meta does not allow both use cases on one app.
  Creators must **reconnect** Facebook after permission or scope changes. Instagram post URLs are not verified via this connect flow.
