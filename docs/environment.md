...
- **Meta (Facebook Login + Instagram):** `META_APP_ID`, `META_APP_SECRET`, `META_REDIRECT_URI` (must be `https://<tunnel>/oauth/facebook/callback` in local dev — **not** `localhost`; Meta rejects `localhost` in App Domains). Register the tunnel host in **App Domains** and **Valid OAuth Redirect URIs**. After callback, the API redirects to `{FRONTEND_URL}/account?oauth=…&platform=…` (first `FRONTEND_URL` origin; `http://localhost:5173` is fine). Scopes include Instagram insights; the creator needs a **professional Instagram** linked to a **Facebook Page** they manage for best results.
